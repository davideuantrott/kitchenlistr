...
- **`renderSameDayLinks(date, currentMeal)`:** iterates `daysBack` 0–4, building a candidate list per day; filters out the current slot and any already-linked slots (prevents chains); groups results into `.link-day-group` blocks with a `.link-day-header` ("Today" in tomato accent, "Yesterday", or "Weekday D Mon" for older days); hides section entirely if no options exist. **Date arithmetic uses `new Date(y, m-1, d)` + local `getFullYear/Month/Date` formatting** to avoid UTC offset shifting dates by one day for UTC+ users (BST etc.)
...
- **Visual layout:** custom `print-visual-header` (Fraunces title + week range + printed date, tomato underline rule); 7-column `print-week-visual` grid; each meal slot shows a colour-coded avatar strip (`recipeAvatarColor()` pastel bg + `recipeAvatarEmoji()` emoji, or `<img>` when `imageUrl` is set) with the recipe name below; empty slots are minimal placeholders
- **Grid fills the page:** `.print-week-visual` set to `height: 490pt` (A4 landscape usable area minus header); day cards are `flex-column`; each `.print-meal-slot` has `flex: 1` so all slots share the card height equally; `.print-visual-avatar` uses `flex: 1` with `min-height: 55pt` so thumbnails grow to fill available slot space rather than a fixed `40pt`
- **Linked meals show thumbnails with overlay:** `getLinkedRecipe()` resolves the target recipe; if found, its avatar/image is rendered at full slot height with a semi-transparent dark bar overlay reading "↩ leftovers"; recipe name shown italic below; falls back to plain italic text if the source meal is missing
...
- **Root cause (two-part):** `formatDate()` uses `.toISOString()` (UTC output), so in UTC+ timezones (e.g. BST, UTC+1) a meal saved for local midnight is stored with a key one day behind (e.g. local June 1 → key `'2026-05-31'`). (1) The hard floor in `shouldIncludeDateInRange` used `new Date(dateStr) < getToday()`, which compared UTC midnight against local midnight and incorrectly treated today's key as past. (2) The date inputs were written with `formatDate` (UTC strings) so they showed yesterday's date, and when the user corrected them to today's local date, `onDateRangeChange` parsed with `new Date(str)` (UTC midnight again), giving a filter string inconsistent with the stored keys.
